...
DATA_FILE: Final[Path] = DATA_DIR / "clinic_data_kjleopold.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "anomalies_kjleopold_custom.csv"
...
def main() -> None:
    ...
    # Create anomalies with explanations
    anomalies_df: pl.DataFrame = df.with_columns(
        pl.when(pl.col("age_years") >= MAX_REASONABLE_X_VALUE)
        ...
        .then(pl.lit("Unrealistic age/height combo"))
        .when(pl.col("height_inches") < 24)
